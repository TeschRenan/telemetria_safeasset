...
async fn worker_sqs(rx: std::sync::mpsc::Receiver<String>) {
    ...
    let queue_name = env::var("QUEUE_FMC920").expect("QUEUE_FMC920 não definido");
        ...
        for event in &events {
            match event.token() {
                ...
                token if event.is_readable() => {
                    ...
                    let ready = requests.get(&token).unwrap().windows(1).any(is_json_end);
                    ...
                    if ready {
                        ...
                        println!("Error to find closing brace '}}'");
